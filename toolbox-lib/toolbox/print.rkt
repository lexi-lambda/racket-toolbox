...
(provide (contract-out
          ...
          [write/mode (-> any/c output-port? custom-write-mode/c void?)]
          [make-write/mode (-> output-port? custom-write-mode/c (->* [any/c] [output-port?] void?))]
...
(define (write/mode v out mode)
  ((make-write/mode out mode) v))

(define (make-write/mode out mode)
  (match mode
...
(define (delimited-printing-sequence vs
  ...
  (custom-printing-value
   (λ (out mode)
     ...
     (write/mode printing-vs out mode)
...
(define printing-append
  (case-lambda
    ...
    [vs (custom-printing-value
         (λ (out mode) (for-each (make-write/mode out mode) vs)))]))
